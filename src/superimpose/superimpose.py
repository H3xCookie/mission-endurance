...
def get_RGB(image: MLModelInput) -> np.ndarray:
    ...
    im_data = image.data 
    return np.stack([im_data[red_indeces[0]], im_data[green_indeces[0]], im_data[blue_indeces[0]]], axis=2)
...
def filter_image(band_filtered_image: SatImage, shapefile: gpd.GeoDataFrame) -> MLModelInput:
    ...
    cropped_image = rioxarray.open_rasterio(band_filtered_image.image).rio.clip(data["geometry"], data.crs)
