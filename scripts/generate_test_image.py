...
def scale_down_ground_image():
    ...
    offset_filename = "./monkedir/sat_image_4_bgr.tiff"
    with rasterio.open(ground_image) as sent_image:
        ...
        offset_height, offset_width = int(0.0 * height), int(0.0 * width)
        meta.update(height=new_height, width=new_width, dtype=np.uint8)
