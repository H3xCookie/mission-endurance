...
def ndvi_of_field(filtered_image: MLModelInput) -> float:
    ...
    average_color = np.average(filtered_image.data, axis=(1, 2))*area/(area - np.average(zero_nums))
    print(average_color)
    ...
    if len(nir_indeces) == 0 or len(red_indeces) == 0:
        print("error")
        raise Exception("to compute ndvi we need the filtered_indeces_image to have band 8 and 4")
